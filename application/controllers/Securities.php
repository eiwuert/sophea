 ...
 * */
	...
	function __construct() {
            ...
            // $this->load->model("Visitor_opt","VisitorOptModel");
	...
	function getPagination($controller, $total, $item_per_page, $page_id) {
	    ...
	    // Clinical Clinical
        $data['h_adults'] = $this->Lang('adults');
        $data['h_opd_adults'] = $this->Lang('opd_adults');
            $data['h_o_ob'] = $this->Lang('o_ob');
            $data['h_o_gen_med'] = $this->Lang('o_gen_med');
            $data['h_o_servical_cancer_screening'] = $this->Lang('o_servical_cancer_screening');
            $data['h_o_cardio'] = $this->Lang('o_cardio');
            $data['h_o_eye'] = $this->Lang('o_eye');
            $data['h_o_pulmonaire'] = $this->Lang('o_pulmonaire');
            $data['h_o_trauma'] = $this->Lang('o_trauma');
            $data['h_o_renal'] = $this->Lang('o_renal');

            $data['h_o_maternity'] = $this->Lang('o_maternity');
            $data['h_o_medicine'] = $this->Lang('o_medicine');
            $data['h_o_gyn'] = $this->Lang('o_gyn');
            $data['h_o_surgery'] = $this->Lang('o_surgery');
            $data['h_o_infertility'] = $this->Lang('o_infertility');
            $data['h_o_orl'] = $this->Lang('o_orl');
            $data['h_o_ent'] = $this->Lang('o_ent');
            $data['h_o_dermatology'] = $this->Lang('o_dermatology');
            $data['h_o_bone'] = $this->Lang('o_bone');
        ...
        $data['h_ipd_adults'] = $this->Lang('ipd_adults');
            ...
            $data['h_i_orl'] = $this->Lang('i_orl');
            $data['h_i_ent'] = $this->Lang('i_ent');
            $data['h_i_dermatology'] = $this->Lang('i_dermatology');
            ...
            $data['h_i_trauma'] = $this->Lang('i_trauma');
            $data['h_i_pulmonaire'] = $this->Lang('i_pulmonaire');
            $data['h_i_renal'] = $this->Lang('i_renal');
            $data['h_i_icu'] = $this->Lang('i_icu');
            $data['h_i_icu_ob'] = $this->Lang('i_icu_ob');
        $data['h_parmacy'] = $this->Lang('parmacy');
        $data['h_echo_ovaries'] = $this->Lang('echo_ovaries');
        $data['h_echoMaternity'] = $this->Lang('echoMaternity');
        $data['h_obExamine'] = $this->Lang('obExamine');
        $data['h_echoOther'] = $this->Lang('echoOther');
        $data['h_anc'] = $this->Lang('anc');
        $data['h_ivg'] = $this->Lang('ivg');
        $data['h_perine'] = $this->Lang('perine');
        $data['h_diagnostic'] = $this->Lang('diagnostic');
        $data['h_eco_other'] = $this->Lang('echo_other');
        $data['h_anc'] = $this->Lang('anc');

        // suppport service
        $data['h_support_service'] = $this->Lang('support_service');
        $data['h_labo'] = $this->Lang('labo');
        $data['h_xray'] = $this->Lang('xray');
        $data['h_ctscan'] = $this->Lang('ctscan');
        $data['h_anapat'] = $this->Lang('anapat');
        $data['h_hpv'] = $this->Lang('hpv');
        $data['h_colpo'] = $this->Lang('colpo');
        $data['h_thinprep'] = $this->Lang('thinprep');
        $data['h_papsmear'] = $this->Lang('papsmear');
        $data['h_x_ray_overay'] = $this->Lang('x_ray_overay');
        $data['h_dna'] = $this->Lang('dna');
        $data['h_ecg'] = $this->Lang('ecg');
        $data['h_gastro_endoscopy'] = $this->Lang('gastro_endoscopy');
        $data['h_other_support_service'] = $this->Lang('other_support_service');

        // echo
        $data['h_eco'] = $this->Lang('eco');
        $data['h_echo_anc'] = $this->Lang('echo_anc');
        $data['h_echo_neo_cardio'] = $this->Lang('echo_neo_cardio');
        $data['h_other_adult_echo'] = $this->Lang('other_adult_echo');

        // booking
        $data['h_booking'] = $this->Lang('booking');
        $data['h_b_opd_booking'] = $this->Lang('b_opd_booking');
        $data['h_b_ipd_booking'] = $this->Lang('b_ipd_booking');

        // service
        $data['h_service'] = $this->Lang('service');
        $data['h_s_examen'] = $this->Lang('s_examen');
        $data['h_s_perine'] = $this->Lang('s_perine');
        $data['h_s_ivg_igm'] = $this->Lang('s_ivg_igm');
        $data['h_s_anc_cpn'] = $this->Lang('s_anc_cpn');
        $data['h_s_miscarrage'] = $this->Lang('s_miscarrage');
        $data['h_s_medicine_abortion'] = $this->Lang('s_medicine_abortion');
        $data['h_s_vpi_vaccination'] = $this->Lang('s_vpi_vaccination');
        $data['h_s_neo_labo'] = $this->Lang('s_neo_labo');
        $data['h_s_bone_test'] = $this->Lang('s_bone_test');
        // Pharmacy
        $data['h_p_pharmacy'] = $this->Lang('p_pharmacy');
        // Pediatric
        $data['h_pediatric'] = $this->Lang('pediatric');
        $data['h_pe_opd_ped'] = $this->Lang('pe_opd_ped');
        $data['h_pe_ped_ipd'] = $this->Lang('pe_ped_ipd');
        $data['h_pe_ped_frencectomy'] = $this->Lang('pe_ped_frencectomy');
        $data['h_pe_ped_circumcision'] = $this->Lang('pe_ped_circumcision');
            $data['h_opd_p'] = $this->Lang('opd_p');
            $data['h_ipd_p'] = $this->Lang('ipd_p');

        // neo
        $data['h_neo'] = $this->Lang('neo');
        $data['h_p_checkNeo'] = $this->Lang('p_checkNeo');
        $data['h_p_chNeoOpd'] = $this->Lang('p_chNeoOpd');
        $data['h_p_chNeoSimpleIcu'] = $this->Lang('p_chNeoSimpleIcu');
        $data['h_p_chNeoComplicatedIcu'] = $this->Lang('p_chNeoComplicatedIcu');
        $data['h_p_chNeoEcho'] = $this->Lang('p_chNeoEcho');
        $data['h_p_chNeoLabo'] = $this->Lang('p_chNeoLabo');
        $data['h_p_pediaOPD'] = $this->Lang('p_pediaOPD');
        $data['h_p_pediaIPD'] = $this->Lang('p_pediaIPD');

        // neonatal
        $data['h_neonatal'] = $this->Lang('neonatal');
        $data['h_n_opd'] = $this->Lang('opd');
        $data['h_n_ipd'] = $this->Lang('ipd');
        $data['h_n_neo_echo'] = $this->lang('neo_echo_cardio');
        $data['h_n_vaccination'] = $this->lang('vaccination');
        $data['h_n_neo_icu'] = $this->lang('neo_icu');
        // $data['h_echo_neo_cardio'] = $this->Lang('echo_neo_cardio');
        // $data['h_other_adult_echo'] = $this->Lang('other_adult_echo');
        ...
        // $data['h_ob_examine'] = $this->Lang('ob_examine');
