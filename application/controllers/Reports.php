    ...
    function serviceCount(){
        ...
        if($this->getPost('serviceOpt') == 1){
              $titleOpd = array("1"=>"OPD", "10"=>"OPD OB", "11"=>"OPD Gen Med", "12"=>"Servical Cancer Screening", "13"=>"OPD Cardio", "14"=>"OPD EYE", "15"=>"OPD Pulmonaire", "16"=>"OPD Trauma", "17"=>"OPD Renal", "18"=>"Maternity", "19"=>"Medicine",
                          "12"=>"GYN", "21"=>"Surgery", "22"=>"Infertility", "23"=>"ORL", "24"=>"ENT", "25"=>"Dermatology", "26"=>"Bone", "27"=>"Digestive", "28"=>"Cardiaque", "29"=>"OPD others", "30"=>"ICU");
              ...
              $titleOpd = array("86"=>"Examen","87"=>"Perine","88"=>"IVG/IGM","89"=>"ANC/CPN","90"=>"Miscarrage","91"=>"Medicine Abortion","92"=>"VPI-Vaccination","93"=>"Neo Labo","94"=>"Bone Test");
        ...
        // ----
              if($queryOpd !== ''){
                  ...
                  foreach ($queryOpd as $key => $value) {
                      $arrCount[$value->visitors_status] = $value->count;
                  }
                  $resultOpd = array_map(null,$idOpd,$arrCount, $titleOpd);
              }else{ $resultOpd = array(); }
              if($queryIpd !== ''){
                  $arrCount = array();
                  foreach ($queryIpd as $key => $value) {
                      $arrCount[$value->visitors_patient_code] = $value->count;
                  }
                  $resultIpd = array_map(null,$idIpd,$arrCount, $titleIpd);
              }else{ $resultIpd = array(); }

        $data['service_report_opd'] = $resultOpd;
        $data['service_report_ipd'] = $resultIpd;
        // Load View
