<div class="content-wrapper" style="min-height: 916px;">
	...
	<section class="content-header">
		<h1>
			<?php echo '<a href="'.$base_url.'index.php/protocols">'.$h_protocol.'</a>';?>
				...
				<!--Protocols Code-->
				  <div class="form-group">
					<div class="input-group">
					  ...
					  <input type="text" name="protocols_title" id="protocols_title" class="form-control">
                      <input type="text" name="protocols_id" id="protocols_id" style="display:none;">
					</div>
				  </div>
				<!-- Desc -->
				  <div class="form-group">
					<div class="input-group">
					  ...
					  <textarea name="protocols_desc" id="protocols_desc" class="form-control" rows="7"></textarea>
					</div>
				  </div>						
...
<script>
    ...
    function getProtocolList(mySearch,pageStart,pageLimit){
        ...
        var i = 0;
        var stRow = '';
        $.post("<?php echo $base_url;?>index.php/protocols/get_protocol_list",{
            ...
            $.each(data, function(key,value) {
                ...
                htmlView += '<tr ' + stRow + '>';
                    // htmlView += '<td>' + i + '</td>';
                    htmlView += '<td>' + value.protocols_title + '</td>';
                    htmlView += '<td>' + value.protocols_desc + '</td>';
                    htmlView += '<td>';
                        htmlView +='<a href="#" title="<?php echo @$edit;?>"><i class="fa fa-edit action-btn primary" onclick="editProtocol(' + value.protocols_id + ');"></i></a>';
                        htmlView +='<a href="#" title="<?php echo @$delete;?>"><i class="fa fa-trash-o action-btn danger" onclick="deleteProtocol(' + value.protocols_id + ');"></i></a>';
                    htmlView += '</td>';
                htmlView += '</tr>';
            });
    ...
    function editProtocol(ids){
		 $('#msgs').css('display','none');
		 $('#form_row').css('display','block');
		 $('#form_table').css('display','none');
		 $('#submit_edit').val('Update');
		 $('#title_name').html('/ Edit');
		 $.post("<?php echo $base_url;?>index.php/protocols/get_protocol_info_by_id_json/"+ids,
		 function(data,status){
			$.each(data, function(key,value) {
				$('#protocols_id').val(value.protocols_id);
				$('#protocols_title').val(value.protocols_title);
				$('#protocols_desc').val(value.protocols_desc);
			});
		 });
    }

    function saveEdit(){
		var protocolsId = $('#protocols_id').val();
		var protocolsTitle = $('#protocols_title').val();
		var protocolsDesc = $('#protocols_desc').val();	
		$.post("<?php echo $base_url;?>index.php/protocols/save_protocols",{
	...
	function deleteProtocol(ids){
		 $.post("<?php echo $base_url;?>index.php/protocols/delete_protocols/"+ids,{protocols_id: ids},function(data,status){
             pagination();
         }); 
        pagination();
   ...
   /* Jquery Pagination */
    function pagination(ids){
		...
		getProtocolList(mySearch,pageStart,pageLimit);
	}
