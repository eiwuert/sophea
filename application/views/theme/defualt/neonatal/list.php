...
<script>
  ...
  function getNeoList(mySearch,pageStart,pageLimit){
	...
	// $( "#neonatal_patient" ).on('keypress keyup',function(){
	// 	var NeoInput = this.value;
	// 	var url = <?//php echo '"'.$base_url.'index.php/patients/patient_auto_complete/"';?>;
	// 	var soc = String(url+NeoInput);
	// 	$( '#neonatal_patient' ).autocomplete({source: soc});

	// 	var dataNeo = $( '#neonatal_patient' ).val();
	// 	var neoSplit = dataNeo.split("=");
	// 	var neoPatientCode = neoSplit[0];

	// 	if(dataNeo !== ""){
	// 	    $.post("<?//php echo $base_url;?>index.php/patients/get_patient_json/"+neoPatientCode,function(data){
	// 			$.each(data, function(key,value) {
	// 			    // $( '#neonatal_patient' ).val(value.patient_en_name);
	// 			    $( '#neonatal_patient_id' ).val(value.patient_id);
	// 			});
	// 	    });
	// 	}
	// });
